...

{
  kakounePlugins = super.kakounePlugins // {
    kakoune-lsp = super.stdenv.mkDerivation {
      pname = self.kakoune-lsp.pname + "-plugin";
      version = self.kakoune-lsp.version;
      phases = ["installPhase"];
      installPhase = ''
	mkdir -p $out/share/kak/autoload/plugins
	echo -e 'eval %sh{${self.kakoune-lsp}/bin/kakoune-lsp --kakoune -s $kak_session}' > $out/share/kak/autoload/plugins/lsp.kak
      '';
      meta = with super.lib; {
